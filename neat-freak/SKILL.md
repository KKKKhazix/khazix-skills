...
- **找到锚点** → 增量模式：
  1. `git diff --name-only $LAST_ANCHOR..HEAD` 获取变更文件列表。
  2. 对每个变更文件读**完整内容**（不只 diff 行），理解语义变化。
  3. **扩散分析**：基于语义变更搜索可能受影响的**未变更**文件和文档——改了接口/签名则 grep 调用者，改了环境变量则 grep 全部引用，改了路由/model 字段则 grep 相关注册和映射。
  4. 盘点范围 = 变更文件 + 扩散文件。步骤 1–3 在此范围内执行；步骤 4–7（修改、验证、汇报）标准不变。
