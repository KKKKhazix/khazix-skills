...
这个 Skill 保持原来的强收尾定位，但执行深度分三档。**如果用户没指定模式，按默认模式执行**。

### `dry-run` mode：只审查，不修改

适用场景：
- 用户想先看哪些文档 / 记忆应该同步
- 用户明确说"先别改文件"、"只列计划"、"先审查一下"

行为规则：
- 可以读 Git 状态、diff 摘要、相关文档和代码
- 可以输出建议修改清单和风险项
- **不得修改任何项目文件**
- **不得创建新文件**
- **不得修改 agent memory**

### 默认模式（`light` mode）：常规收尾同步
...
行为规则：
- 仍然按原 skill 的方式做收尾同步
- 重点聚焦**本次改动直接影响**的 `README.md`、`CLAUDE.md` / `AGENTS.md`、相关 `docs/` 与记忆
- 不做无关的大范围文档重写

### `full` mode：完整收尾 / 交接模式

适用场景：
- 用户明确要求完整收尾、交接准备、新人接手、全量核对
- 用户执行 `/neat --full`

行为规则：
- 执行完整盘点
- 读取所有项目根 markdown 与 `docs/*.md`
- 按 [references/sync-matrix.md](references/sync-matrix.md) 做全量检查
- 明确检查跨项目影响、过期内容、重复内容、相对时间
...
不要把通用学习笔记、教程解释、用户这轮为了理解概念而展开的推理过程写进项目文档。

以下内容默认**不进入** `README.md` / `docs/` / `CLAUDE.md` / `AGENTS.md`：
- 某个框架或工具的通用概念解释
- 用户本轮学习过程中的提问与回答
- 临时调试时的思路碎片
- 尚未稳定采用的方案草稿
...
只有当这些内容已经沉淀为**项目事实**时，才允许写入文档，例如：
- 本项目为什么选择某个架构或约束
- 本项目某个模块 / API / 环境变量 / 工作流现在应该怎么用
- 新人接手项目时必须知道的规则、踩坑结论、运维事实
...
规则：
- 先识别**这次同步之前就已经存在**的脏改动
- 不要把 pre-existing dirty changes 自动打包进这次同步
- 如果工作区里本来就有未提交改动，先在内部标记风险，再只处理本次需要同步的事实
- `dry-run` mode 可以读取这些信息，但**仍然不能改文件**
...
**写入前先验真**：命令、路径、环境变量名、路由 / API 名、模块名，只要要写进文档，就先从代码、配置或真实文件结构里核对。**先验证，再写入**，不要把猜测、脑补或上文里顺口说的名字直接落成项目事实。
...
**完整性 / 反漏改（再查这组）**：
- [ ] 第二步列出的每个文件，都判断了"不用改"或"已改"
- [ ] 记忆索引（若有）里的每个链接指向存在的文件
- [ ] 每个记忆文件的 description 和内容对得上
- [ ] 记忆之间没有互相矛盾
- [ ] CLAUDE.md / AGENTS.md 里提到的路径 / 命令 / 工具 / 环境变量 / 路由 / API 名在代码或配置中真实存在
...
**跨项目改动**：本次对话改了多个项目，每个项目都要跑一次完整的第二步（ls + 读 docs）。不要假设一个项目的 docs 改了，另一个就不用。尤其是上游-下游对接文档（集成指南 / SDK 说明 / API 协议），两边都要对齐。
