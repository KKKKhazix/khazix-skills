...
- **找到锚点** → 增量模式（替代步骤 0 的全量枚举和步骤 3 的路由搜索）：
  1. `git diff --name-only $LAST_ANCHOR..HEAD` 获取变更文件列表——不枚举未变更文件。
  2. 读变更文件的完整内容（不只 diff 行），理解语义变化。
  3. **扩散分析**（融合步骤 3 职责）：基于语义变更搜索可能受影响的**未变更**文件和文档——改了接口/签名则 grep 调用者，改了环境变量则 grep 全部引用，改了路由/model 字段则 grep 相关注册和映射。映射参考 [references/sync-matrix.md](references/sync-matrix.md)。
  4. 盘点范围 = 变更文件 + 扩散文件。步骤 1–2（事实矩阵、规则审计）在此范围内执行；步骤 4–7（修改、验证、汇报）标准不变。增量模式下步骤 3 已由扩散分析覆盖，无需重复执行。
- **未找到** → 全量模式：按步骤 0–7 完整执行，行为不变。
...
- [ ] 全部文件已机械枚举（增量模式下：变更文件 + 扩散文件已覆盖）；受影响文件已阅读并作出「改/不改」判断。
