...
description: |
  ...
  当用户想要系统性研究一个产品、公司、概念、技术或人物时使用。核心是双轴分析：纵轴追踪从诞生到当下的完整生命历程（以叙事故事呈现），横轴在当下时间截面上与竞品/同类进行系统性横向对比，最后交叉两条轴产出独到洞察。
  内置：三级证据标注体系（✅⚠️❌）、五级工具升级链（WebSearch→Agent→API→CDP→人工）、平台专用搜法速查、暂缺清单（强制输出）、面试模式（第一性原理拆解→三段论回答→案例+注意点→北极星收束→反思问题）。
  触发词包括但不限于：横纵分析、研究一下、帮我分析、深度研究、做个研究、调研一下、竞品分析、帮我看看这个东西怎么样、这个产品/公司/概念是怎么回事、帮我摸清楚、帮我搞懂、帮我做个deep research、面试准备、帮我准备面试。
...
搜索完成后，对收集到的每一条信息进行三级标注。**报告正文中关键判断必须携带这些标签**：

| 标注 | 含义 | 示例 |
|:--:|------|------|
| ✅ | 一手数据或多源二手来源交叉验证 | "KS众筹$46.76M（CDP浏览器直接提取+多家媒体报道）" |
| ⚠️ | 基于部分数据的推理或单一二手来源 | "官方沟通偏向公告模式（从截图推断，需到岗后确认）" |
| ❌ | 无法从公开渠道获取，需人工/内部验证 | "内容团队规模——完全未知" |

**原则**：你可以有不带标签的叙述性文字（故事、过渡、分析），但每一个**事实性断言**必须在上下文中能找到对应的标签。不确定的事，说"我不确定"——这比瞎编好一百倍。

### 工具升级链（L1→L5）

不是所有信息都能用同一种方法拿到。按以下顺序逐级升级，一步失败就换下一步：

| 层级 | 方式 | 适用场景 | 局限性 |
|------|------|------|------|
| **L1: WebSearch** | 搜索引擎 | 发现信息源、找二手分析文章 | 不直接给一手数据；**不能用搜索引擎搜社交媒体账号**（如搜"TikTok eufyMake"——隔了一层，命中率极低） |
| **L2: Agent 分派** | 多路并行 Agent | 覆盖不同维度（纵向/横向/竞品） | Agent搜不到的不等于不存在 |
| **L3: 公开API/工具** | yt-dlp、B站API、网站JSON接口 | YouTube频道、B站用户搜索、结构化数据 | 可能被反爬/风控 |
| **L4: CDP浏览器** | web-access skill | JS渲染页面（Kickstarter、电商、TikTok） | 需要Chrome remote debugging |
| **L5: 人工浏览** | 让用户帮忙截图/访问 | 一切能手动访问的页面 | 最可靠，但需要用户配合 |

**每次工具失败时**，必须判断属于哪种失败模式：
- 工具被拦截（如Reddit 403、YouTube反爬"sign in to confirm not bot"）
- 方法本身就错了（如用搜索引擎搜社交媒体账号）
- 确实没有（L3+L4都验证为空 → 可以判断为"未发现"）

### 平台专用搜法速查

不要对所有平台用同一种搜法。以下为验证过的方法：

| 想查什么 | 正确方法 | 错误方法 |
|------|------|------|
| TikTok 是否有账号 | 直接访问 `tiktok.com/@handle` 验证 → CDP浏览器 → ScrapeCreators | ❌ WebSearch搜"TikTok eufyMake" |
| B站 是否有账号 | B站公开API `api.bilibili.com/x/web-interface/search/type?search_type=bili_user&keyword=XXX` → space.bilibili.com验证 | ❌ 用搜索引擎搜 |
| Reddit 子版块 | **人工浏览最可靠**；old.reddit.com偶尔可行；ScrapeCreators有Reddit API；CDP浏览器 | ❌ WebFetch直接请求reddit.com（被403拦截） |
| YouTube 频道列表 | yt-dlp `--flat-playlist` | — |
| YouTube 播放量 | yt-dlp + Chrome登录态cookie (`--cookies-from-browser chrome`) | ❌ 未登录的yt-dlp请求详情 |
| Kickstarter 详情 | CDP浏览器（web-access skill） | ❌ WebFetch（动态渲染） |
| 公司财务 | 上市公司公开财报 | — |
...
- **数据造假是调研最大的罪**：为追求表达的锋利而凭空捏造具体数字（如"只有一个人在维护""播放量约XXX"），比遗漏数据严重100倍。写任何数字之前问自己：这个数字我是从哪来的？答不上来，就不写。"我不知道"不是失败，把"我不知道"包装成"我知道"才是。
- **"搜不到" ≠ "不存在"**：这是两种完全不同的状态。工具被拦截（如WebFetch遇到403）、方法本身就错了（如用搜索引擎搜TikTok账号）、或者确实没有——每次信息获取失败，必须区分属于哪一种。不要把工具的盲区说成事实的结论。
...
五、数据缺口与暂缺清单（强制）
[列出所有标注为 ❌ 的信息——哪些是我们想查但没查到的、哪些需要人工/内部验证、哪些工具被拦截了但目标可能存在。这不是"失败清单"，是"下一步该查什么"的路线图]

六、信息来源
[所有引用的来源列表，每条标注证据等级 ✅/⚠️/❌]
```

**暂缺清单的价值**：一份研究报告的边界，定义在"我们不知道什么"上。公开信息的边界到了就说"到了"。内容团队有几个人的答案不在互联网上，在内部系统里——提前说"这需要到岗后验证"不是功课不足，是专业。

### 文件命名和交付

PDF文件命名为 `[研究对象名称]_横纵分析报告.pdf`，保存到用户的工作目录中。

**HTML备选输出**：首次生成PDF后，将同一份Markdown转为HTML并在浏览器打开。HTML打开成功率远高于PDF（不依赖系统字体），作为兜底交付方式。转换命令：`python3 -c "import markdown; ..."` 生成带CSS样式的HTML文件。

---

## 产出形态适配

同一份调研数据，根据用户的需求场景选择不同的输出形态。**不要在信息收集前决定产出形态——信息收完之后，根据"用户到底要用在哪"来选。**

| 产出形态 | 适用场景 | 特点 |
|------|------|------|
| **研究报告**（默认） | "帮我研究一下这个公司/产品" | 完整纵轴+横轴+交汇，10,000-30,000字，叙事驱动 |
| **面试准备手册** | "我要去这家公司面试" | 问答形式+数据速查表+反思策略。每道回答按"第一性原理拆解→三段论回答→案例+注意点→北极星收束"组织。篇幅 5,000-8,000 字。核心原则：把"我调研了什么"变成"面试中我能用什么"，把"我觉得他们做得怎么样"变成"我会怎么做"。 |
| **决策建议** | "我要决定投不投/买不买/做不做" | 只保留结论+关键证据+三个未来剧本，不要中间推理过程 |
| **数据速查表** | 面试前/开会前快速过一眼 | 一页纸，只放 ✅ 标注的数据，不放分析和推测 |

**面试模式的额外规则**：

面试回答的底层逻辑：**面试官想听到的不是"答案"，是"你怎么思考的"。** 每个问题字面意思下面都藏着一个没说出口的验证需求——找到它、回答它，才是有效的面试回答。
...
1. **字面问题**：他问了什么？
2. **实际验证**：他真正在判断什么？（通常 2-3 个验证维度）
3. **第一性原理**：这个岗位要解决的本质问题是什么？从这个原理出发倒推回答。

例：问"你怎么设计布道体系？" → 验证：你有没有体系化思维？你能不能区分不同人的不同障碍？你知不知道自己的边界？ → 第一性原理：人不是因为"学会了"而改变行为，是因为"新的方式明显更省事"才改变。

#### 第二步：三段论回答结构

| 段位 | 占比 | 作用 | 内容 |
|:---:|:---:|------|------|
| **第一段** | 10% | 建立认知锚点 | 一句话亮出你的核心判断——"我先说一个判断——..." |
| **第二段** | 60% | 证明判断成立 | 逻辑展开 + 具体案例 + 注意的地方 |
| **第三段** | 30% | 扣回对方需求 | 落回岗位："所以我入职前90天的落地节奏是..." + 北极星目标 |

#### 第三步：案例+注意点（实操驱动）

理论框架只占 30-40%，实操案例占 60-70%。每个策略点必须：
- **案例**：用具体的人、具体的问题、具体的数字、具体的结果。不是"销售回邮件太慢"，是"销售团队每天花 2-3 小时写客户跟进邮件，我帮两个人搭了模板，一周后 8 个人都在用"。
- **注意的地方**：每个案例后面跟注意事项。包括前提条件、常见误区、什么情况下不适用。案例证明你做过了，注意事项证明你踩过坑。

#### 第四步：北极星收束

每道回答必须落到一个可被验证的目标上。格式："北极星目标不是 A，而是 B。因为 A 是手段/过程/中间产物，B 才是结果。" 如："北极星不是'做了多少场培训'，而是'在一个业务场景里跑通了行为变化→案例传播→主动想用的闭环'。"

#### 面试回答的四条硬约束

1. **不做评判。** 你不知道对方的参与度、资源约束、谁做的决策。用"我的做法是..."替代"你们的问题是..."。除非被直接问"你觉得我们做得怎么样"，绝对不提。
2. **数据源必须是系统日志，不能是人工上报。** 凡是需要填表、填问卷、人工定义的指标，造假成本太低，不作核心依据。读不了的指标换一个能读的，不要硬凑。
3. **翻译分层。** 同一批素材，给行业内看技术复盘，给人才看团队叙事，给大众看具体故事。不是一个内容覆盖所有人，是一个内容源切三份。
4. **最好的体系设计是让自己变得不必要。** 角色随阶段退出：攻坚手 → 制片人 → 数据哨兵。终极成功不是培训讲得好，是三个月后这岗位不再被需要。

#### 反问环节

给出能同时测三类信息的问题模板：
- **测 AI 理解深度**："公司 AI 培训最大的挑战是不想用、不会用、还是不敢用？"
- **测岗位权限**："前三个月的北极星目标您觉得应该是什么？"
- **测匹配度**："您心目中这个岗位做得特别好的人是什么样子的？"

**简历绑定**：如果用户上传了简历，必须将回答与用户的真实经历绑定——不用通用模板。
**面试模式触发条件**：用户在请求中明确提到"面试""应聘""准备面试"等关键词，或研究动机是面试准备。

**❗面试模式提醒**：当检测到用户的研究目的是面试时，主动提醒：**"如果你需要更贴合你个人背景的面试回答，可以上传你的简历，我会把回答绑定到你的真实经历上。"**
...
- [ ] 每个事实性断言有证据等级标注（✅⚠️❌）？
- [ ] "暂缺清单"列出了所有 ❌ 项目？不只是形式化地写一条，是真的把不知道的东西列出来了？
- [ ] 有没有任何数字/事实/判断是编的而非查的？（写每个数字之前问：这个数字我从哪来的？）
- [ ] 有没有把"搜不到"等同于"不存在"？（区分工具被拦截 vs 确实没有 vs 方法错了）
- [ ] 所有关键事实有可靠来源支撑？有没有只靠单一来源就下判断的？
- [ ] 搜不到的信息诚实标注了「暂缺」，没有编造？
...
- [ ] 没有触犯绝对禁区里的任何一条？
- [ ] PDF排版美观、结构清晰、可读性好？（如PDF打不开，自动降级为HTML）?
- [ ] 总字数在合理范围内？
