...
- **Windows**：扫 user_profile、appdata_local、appdata_roaming、temp、downloads、program_files(_x86)、dev_caches、**windows_temp、windows_logs、windows_softdist、windows_installer、windows_winsxs、windows_driverstore**（新增系统级目录），用 `os.scandir` 算大小；`system.disks` 含所有盘符。
...
对于新增扫描的 Windows 系统级目录（`windows_temp`、`windows_logs`、`windows_softdist`、`windows_installer`、`windows_winsxs`、`windows_driverstore`），需要特殊处理：

**系统临时文件和日志（`windows_temp`、`windows_logs`）：**
- 🟢 绿灯：可以安全清理，但**不要给一键删除按钮**
- 原因：需要管理员权限，可能被系统占用
- 处理方式：在 `summary.long_term` 写建议："运行磁盘清理工具 `cleanmgr` 清理系统临时文件和日志"

**Windows Update 缓存（`windows_softdist`）：**
- 🟢 绿灯：安全清理
- 处理方式：在 `summary.long_term` 写："Windows 更新缓存可通过磁盘清理工具清理，或手动停止 Windows Update 服务后删除 `C:\Windows\SoftwareDistribution\Download`"

**Windows Installer 目录（`windows_installer`）：**
- 🟡 黄灯：**需要专用工具识别孤立残留**
- 不可盲删：.msi/.msp 文件可能仍被已安装软件引用
- 处理方式：
  ```
  "name": "Windows Installer 缓存",
  "path": "C:\Windows\Installer",
  "size": "约 X GB",
  "description": "Windows 安装包缓存，包含 .msi/.msp 文件。部分文件可能是已卸载软件的孤立残留，但不可盲删",
  "risk": "误删可能导致软件修复/卸载功能失效",
  "how_to_clean": [
    "推荐使用 PatchCleaner 工具（https://www.homedev.com.au/Free/PatchCleaner）",
    "该工具通过检查注册表识别真正的孤立文件",
    "手动删除有风险，务必使用专用工具"
  ],
  "trash_paths": []
...
- 处理方式：
  ...
  "size": "约 X GB",
  "description": "Windows 并行组件存储（Side-by-Side），系统核心目录",
  "why_caution": "手动删除会导致系统崩溃，绝对禁止直接删除",
  "indirect_release": [
    "使用 DISM 命令安全清理：`DISM /Online /Cleanup-Image /StartComponentCleanup`",
    "深度清理（慎用）：`DISM /Online /Cleanup-Image /StartComponentCleanup /ResetBase`",
    "该命令会删除旧版本组件，释放数 GB 空间",
    "需要管理员权限，清理过程需5-20分钟"
  ]
  ```

**驱动程序仓库（`windows_driverstore`）：**
- 🟡 黄灯：需要专用工具选择性清理
- 处理方式：
  ```
  "name": "驱动程序历史版本",
  ...
  "size": "约 X GB",
  "description": "显卡、打印机等驱动的历史版本，每次更新都会保留旧版本",
  "risk": "误删当前使用的驱动会导致硬件无法工作",
  "how_to_clean": [
    "推荐使用 Driver Store Explorer (RAPR) 工具",
    "下载地址：https://github.com/lostindark/DriverStoreExplorer",
    "可视化显示每个驱动的版本、日期、大小，安全选择删除旧版本",
    "典型场景：NVIDIA 显卡驱动历史版本可能占用数 GB"
  ],
  "trash_paths": []
  ```
...
如果某个用户目录（如开发缓存）特别大且可以迁移到其他盘符，在 `summary.long_term` 中添加符号链接迁移建议：

```
"符号链接迁移：对于占用大量空间的目录（如 .cache、.gradle 等），可以移动到 D 盘并创建符号链接。示例：`move C:\Users\username\.cache D:\cache_relocated` 然后 `mklink /D C:\Users\username\.cache D:\cache_relocated`（需管理员权限）"
```

适合迁移的目录：`.cache`、`.npm`、`.gradle`、`.m2`、`AppData\Local\Temp`（但需注意性能影响）
